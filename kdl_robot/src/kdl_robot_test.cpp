...
int main(int argc, char **argv)
{
    ...
    // Gains
    //double Kp = 50, Kd = sqrt(Kp);        //PID1
    //double Kp = 100, Kd = sqrt(Kp);       //PID2
    double Kp = 20, Kd = 4;                 //PID3
    //double Kp = 50, Kd = 0.5;             //PID4
    //double Kp = 50, Kd = 5;               //PID5
    //double Kp = 40, Kd = 30;              //PID6
    //double Kp = 40, Kd = 15;              //PID7
    ...
    while ((ros::Time::now()-begin).toSec() < 2*traj_duration + init_time_slot)
    {
        if (robot_state_available)
        {
            ...
            std::cout << "time: " << t << " s: " << s << " dot_s: "<< dot_s << " ddot_s: " << ddot_s << std::endl;
            ...
            // Cartesian space inverse dynamics control --> Point 4
